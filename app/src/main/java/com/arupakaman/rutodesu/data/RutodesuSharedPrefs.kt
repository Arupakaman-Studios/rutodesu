package com.arupakaman.rutodesu.data
...
class RutodesuSharedPrefs private constructor(mPrefs: SharedPreferences){

    var selectedLanguageCode by mPrefs.dataNullable<String?>()

    var selectedLanguageName by mPrefs.dataNullable<String?>()

    var selectedThemeMode by mPrefs.data(0)

    var checkRootCount by mPrefs.data(0)


    companion object{

        private var mPrefs: RutodesuSharedPrefs? = null

        fun getInstance(mContext: Context): RutodesuSharedPrefs {
            if (mPrefs == null)
                mPrefs = RutodesuSharedPrefs(mContext.getSharedPreferences("RutodesuSharedPrefs", Context.MODE_PRIVATE))
            return mPrefs!!
        }
